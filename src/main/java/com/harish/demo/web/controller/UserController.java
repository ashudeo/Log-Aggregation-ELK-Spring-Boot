package com.harish.demo.web.controller;
...
import com.harish.demo.service.UserService;
import com.harish.demo.service.converter.UserResourceConverter;
import com.harish.demo.web.request.UserResourceRequest;
import com.harish.demo.web.response.ResourceIdResponse;
import com.harish.demo.web.response.SuccessResponse;
...
@RequestMapping("/users") class UserController {
	...
	public String createUser(@Valid @RequestBody UserResourceRequest request) {
		...
		User userId = this.userService.createUser(this.converter.convert(request));
		return toJson(new UserResourceResponse(userId.getId(),userId.getFirstName(),userId.getLastName(),userId.getCountry()));
	}
